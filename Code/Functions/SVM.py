...
def dual_wrapper(D, H, bounds):
    def LDual(alpha, H):
        Ha = numpy.dot(H, functions.vcol(alpha))
        aHa = numpy.dot(functions.vrow(alpha), Ha)
        a1 = alpha.sum()
...
class SVM:
    ...
    def trainClassifier(self, D, L, type='linear', pi=0, balanced=False, K=1, C=0, c=0, d=2, gamma=0):
        ...
        if(type == 'linear'):
            ...
            H = numpy.dot(functions.vcol(self.Z), functions.vrow(self.Z)) * H
            alphaStar = dual_wrapper(D, H, self.bounds)
            self.w = numpy.dot(DTRT, functions.vcol(alphaStar) * functions.vcol(self.Z)).sum(axis = 1)
        if(type == 'RBF'):
            self.gamma = gamma
            Dist = functions.vcol((D ** 2).sum(0)) + functions.vrow((D ** 2).sum(0)) - 2 * numpy.dot(D.T, D)
            kernel = numpy.exp(-self.gamma * Dist) + (self.K ** 2)
            H = numpy.dot(functions.vcol(self.Z), functions.vrow(self.Z)) * kernel
    ...
    def computeLLR(self, D):
        ...
        if(self.type == 'RBF'):
            Dist = functions.vcol((self.DTR ** 2).sum(0)) + functions.vrow((D ** 2).sum(0)) - 2 * numpy.dot(self.DTR.T, D)
            kernel = numpy.exp(-self.gamma * Dist) + (self.K ** 2)
