...
from Functions.GaussianClassifier import logpdf_GAU_ND
...
def GMM_LBG(D, alpha, components, psi, type):
    gmm = [(1, functions.empirical_mean(D), functions.empirical_covariance(D))]
...
def GMM_EM(DT, gmm, psi, type, diff = 1e-6):
    ...
    while to == None or tn - to > diff:
        ...
        newGmm = []
        sigmaTD = numpy.zeros((D,D))
        for i in range(len(gmm)):
            gamma = P[i, :]
            Z = gamma.sum()
            F = (functions.vrow(gamma) * DT).sum(1)
            S = numpy.dot(DT, (functions.vrow(gamma) * DT).T)
            w = Z/P.sum()
            mu = functions.vcol(F / Z)
            ...
            s[s<psi] = psi
            sigma = numpy.dot(U, functions.vcol(s) * U.T)
            newGmm.append((w, mu, sigma))

        if type == "tied":
            sigmaTD /= N
            U, s, _ = numpy.linalg.svd(sigmaTD)
            s[s<psi] = psi
            sigmaTD = numpy.dot(U, functions.vcol(s) * U.T)
            newGmm2 = []
            for i in range(len(newGmm)):
                (w, mu) = newGmm[i]
                newGmm2.append((w, mu, sigmaTD))
            newGmm = newGmm2
        gmm = newGmm
